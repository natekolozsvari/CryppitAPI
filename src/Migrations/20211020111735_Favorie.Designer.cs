// <auto-generated />
using CryppitBackend.Services;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace CryppitBackend.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20211020111735_Favorie")]
    partial class Favorie
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.11")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("CryppitBackend.Models.Crypto", b =>
                {
                    b.Property<string>("FavortieId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<double>("Ath")
                        .HasColumnType("float");

                    b.Property<double>("Change")
                        .HasColumnType("float");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("High24H")
                        .HasColumnType("float");

                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Image")
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("Low24H")
                        .HasColumnType("float");

                    b.Property<double>("MarketCap")
                        .HasColumnType("float");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Symbol")
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("TotalVolume")
                        .HasColumnType("float");

                    b.Property<double>("Value")
                        .HasColumnType("float");

                    b.HasKey("FavortieId");

                    b.ToTable("Crypto");

                    b.HasDiscriminator<string>("Discriminator").HasValue("Crypto");
                });

            modelBuilder.Entity("CryppitBackend.Models.DailyCrypto", b =>
                {
                    b.Property<long>("Changed")
                        .HasColumnType("bigint");

                    b.Property<string>("DetailsFavortieId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasIndex("DetailsFavortieId");

                    b.ToTable("Daily");
                });

            modelBuilder.Entity("CryppitBackend.Models.Investment", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<double>("Amount")
                        .HasColumnType("float");

                    b.Property<string>("CryptoId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("CurrentPrice")
                        .HasColumnType("float");

                    b.Property<double>("PriceBought")
                        .HasColumnType("float");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Investments");
                });

            modelBuilder.Entity("CryppitBackend.Models.User", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<double>("Balance")
                        .HasColumnType("float");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("JoinDate")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = "c784662e0c27497eb4337cb0b2109823",
                            Balance = 1964.21,
                            Email = "john@doe.com",
                            JoinDate = "Wednesday, September 15, 2021",
                            Name = "John Doe",
                            Password = "johndoe"
                        });
                });

            modelBuilder.Entity("CryppitBackend.Models.Favorite", b =>
                {
                    b.HasBaseType("CryppitBackend.Models.Crypto");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasDiscriminator().HasValue("Favorite");
                });

            modelBuilder.Entity("CryppitBackend.Models.DailyCrypto", b =>
                {
                    b.HasOne("CryppitBackend.Models.Crypto", "Details")
                        .WithMany()
                        .HasForeignKey("DetailsFavortieId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Details");
                });
#pragma warning restore 612, 618
        }
    }
}
